...
class Engine(EngineBase):
    ...
    async def async_generate_single_with_tools(
        self,
        tools: List[Tool],
        tools_server: Callable[[List[ToolCallItem]], str],
        tools_stop_tokens_ids: Optional[List[int]] = None,
        ...
        session_id = await self.tokenizer_manager.open_session(obj=OpenSessionReqInput(capacity_of_str_len=-1))
        ...
        while True:
            ...
            block = await generator.__anext__()
            meta_info = block["meta_info"]
            rid = meta_info["id"]
        ...
        await self.tokenizer_manager.close_session(obj=CloseSessionReqInput(session_id=session_id))
